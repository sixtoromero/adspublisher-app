...
@Injectable({
  ...
  export class CategoriasPorMicroEmpresasService extends BaseService<CategoriaModel, MasterModel> {
    ...
    GetCategoriasporMicroempresas(token: string, ID: number) {
      return new Promise( resolve => {
        this.get(APIENDPOINT.getCategoriasporMicroempresas + '?ID=' + ID, true, token)
          .subscribe(resp => {
            if (resp.IsSuccess) {
              resolve(resp.Data);
            } else {
